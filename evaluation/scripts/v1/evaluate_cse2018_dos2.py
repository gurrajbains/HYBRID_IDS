...
DATASET_PATH = Path("evaluation/datasets/CSE-CIC-IDS2018/Friday-16-02-2018_TrafficForML_CICFlowMeter.csv")
MODEL_PATH = Path("models/v1/random_forest_live.joblib")
...
def load_data():
    print("Loading CSE-CIC-IDS2018 Hulk/SlowHTTPTest data...")
...
def create_mapped_ground_truth(labels):
    def map_label(label):
        ...
        if label in {"DoS attacks-Hulk", "DoS attacks-SlowHTTPTest"}:
            return "DoS"
...
def print_attack_breakdown(y_original, predictions):
    for attack_label in ["DoS attacks-Hulk", "DoS attacks-SlowHTTPTest"]:
